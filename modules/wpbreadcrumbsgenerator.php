...
function wp_breadcrumbs_generator() {
  ...
  $error_404          = __('ERROR 404', TEXTDOMAIN);
  $search_results_for = __('Search results for', TEXTDOMAIN);
  $author             = __('Author', TEXTDOMAIN);
  $archive            = __('Archive', TEXTDOMAIN);
  $paginated          = __('page', TEXTDOMAIN);
  ...
  if ( !is_front_page() ) {
    ...
    if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
      ...
      if(!empty($category)) {
        ...
        $category_all_values = array_values($category);
        $last_category = end($category_all_values);
      ...
      // Day display
      echo '<li class="item-current item-' . get_the_time('j') . '"><span class="bread-current bread-' . get_the_time('j') . '"> ' . get_the_time('jS') . ' ' . get_the_time('M') . ' ' . $archive . '</span></li>';
      ...
      // Month display
      echo '<li class="item-month item-month-' . get_the_time('m') . '"><span class="bread-month bread-month-' . get_the_time('m') . '" title="' . get_the_time('M') . '">' . get_the_time('M') . ' ' . $archive . '</span></li>';

    } else if ( is_year() ) {

      // Display year archive
      echo '<li class="item-current item-current-' . get_the_time('Y') . '"><span class="bread-current bread-current-' . get_the_time('Y') . '" title="' . get_the_time('Y') . '">' . get_the_time('Y') . ' ' . $archive . '</span></li>';
      ...
      // Paginated archives
      echo '<li class="item-current item-current-' . get_query_var('paged') . '"><span class="bread-current bread-current-' . get_query_var('paged') . '" title="' . $paginated . ' ' . get_query_var('paged') . '">' . $paginated . ' ' . get_query_var('paged') . '</span></li>';

    } else if ( is_search() ) {

      // Search results page
      echo '<li class="item-current item-current-' . get_search_query() . '"><span class="bread-current bread-current-' . get_search_query() . '" title="' . $search_results_for . ': ' . get_search_query() . '">'.  $search_results_for . ': "' .  get_search_query() . '"</span></li>';

    } elseif ( is_404() ) {

      // 404 page
      echo '<li class="item-current item-404">' . $error_404 . '</li>';
    }
